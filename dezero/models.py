...
class MLP(Model):
    # fc_output_size(fully connected layer): tuple or list
    # ex: fc_ouput_size = (10, 10, 1)
    # -> first layer(L) output: 10, second L output: 10, third L output: 1
    def __init__(self, fc_output_size, activation=F.sigmoid):
        ...
        for i, out_size in enumerate(fc_output_size):
            ...
            setattr(self, "l" + str(i), layer)  # ex) self.l1 = L.Linear(out_size)
            self.layers.append(layer)
...
class VGG16(Model):
    ...
    def __init__(self, pretrain=False):
        ...
        self.pool1 = L.MaxPool(kernel_size=2)

        self.conv2_1 = L.Conv2d(128, kernel_size=3, stride=1, pad=1)
        self.conv2_2 = L.Conv2d(128, kernel_size=3, stride=1, pad=1)
        self.pool2 = L.MaxPool(kernel_size=2)

        self.conv3_1 = L.Conv2d(256, kernel_size=3, stride=1, pad=1)
        self.conv3_2 = L.Conv2d(256, kernel_size=3, stride=1, pad=1)
        self.conv3_3 = L.Conv2d(256, kernel_size=3, stride=1, pad=1)
        self.pool3 = L.MaxPool(kernel_size=2)

        self.conv4_1 = L.Conv2d(512, kernel_size=3, stride=1, pad=1)
        self.conv4_2 = L.Conv2d(512, kernel_size=3, stride=1, pad=1)
        self.conv4_3 = L.Conv2d(512, kernel_size=3, stride=1, pad=1)
        self.pool4 = L.MaxPool(kernel_size=2)

        self.conv5_1 = L.Conv2d(512, kernel_size=3, stride=1, pad=1)
        self.conv5_2 = L.Conv2d(512, kernel_size=3, stride=1, pad=1)
        self.conv5_3 = L.Conv2d(512, kernel_size=3, stride=1, pad=1)
        self.pool5 = L.MaxPool(kernel_size=2)
...
# =============================================================================
# simple example: TwoLayerNet / SimpleCNN / SimpleRNN / SimpleLSTM
# =============================================================================
...
class SimpleCNN(Model):
    def __init__(self):
        super().__init__()
        self.conv1 = L.Conv2d(32, kernel_size=5, stride=1, pad=2)
        self.pool1 = L.MaxPool(2)
        self.conv2 = L.Conv2d(64, kernel_size=3, stride=1, pad=1)
        self.pool2 = L.MaxPool(2)
    ...
    def forward(self, x):
        ...
        x = self.pool2(x)
        x = F.dropout(x, droupout_ratio=0.25)
        x = F.flatten(x)
        x = F.relu(self.fc1(x))
        x = F.dropout(x, droupout_ratio=0.5)
        x = self.fc2(x)
