...
def _dot_var(v, verbose=False):
    ...
    if verbose and v.data is not None:
        if v.name is not None:
            name += ": "
        name += "shape=" + str(v.shape) + " dtype=" + str(v.dtype)
...
def _dot_func(f):
    ...
    for output in f.outputs:
        txt += dot_edge.format(id(f), id(output()))  # output - weakref
    return txt
...
def get_dot_graph(output, verbose=False):
    txt = ""
    funcs = []
    seen_set = set()  # set of operator instances (Mul, Add, ...)
...
def plot_dot_graph(output, verbose=True, to_file="graph.png"):
    ...
    tmp_dir = os.path.join(os.path.expanduser("~"), "./Desktop/DeZero")
    ...
    # dot file -> png
    extension = os.path.splitext(to_file)[1][1:]  # 확장자 분리
...
def show_progress(block_num, block_size, total_size):
    ...
    if i >= 30:
        i = 30
    bar = "#" * i + "." * (30 - i)
    # end = "", the cursor stay on the same line and progress bar get overwritten while updated
...
def get_file(url, file_name=None):
    ...
    if file_name is None:
        # extract file name from the given url
    ...
    if os.path.exists(file_path):  # check the file already exists
        return file_path

    # if the file does not exist in the cache_dir, download it
